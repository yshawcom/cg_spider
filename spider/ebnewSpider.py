...
class EbnewSpider:
    ...
    def __init__(self):
        self.log = LogHandler(ebnewConst.NAME)
    ...
    def parse_detail_html(self, ori_id, html):
        ...
        if len(div_ebnew_details_contents) <= 0:
            self.log.info('---------------- 公告 %s 未解析出数据', ori_id)
            self.log.info('')
            return
        ...
        self.log.info('公告名称: %s', details_title)
        ...
        self.log.info('发布时间: %s', release_time)
        ...
        self.log.info('公告类型: %s', announcement_type)
        ...
        self.log.info('招标地区: %s', tender_area)
        ...
        self.log.info('招标产品: %s', tender_products)
        ...
        details_content = str(div_details_content) \
            ...
            .replace('	', '') \
            .replace('  ', ' ')
        # self.log.info('主体内容: %s', details_content)
        ...
        ebnewDao.save(model)
        self.log.info('---------------- 公告 %s 已保存到数据库', ori_id)
        self.log.info('')
    ...
    def request_detail(self, url, cookies=None):
        ...
        self.log.info('---------------- 公告ID: %s', ori_id)

        # 判断数据库是否已存在相同id数据
        notice = ebnewDao.find_top_by_ori_id(int(ori_id))
        if notice is not None:
            self.log.info('---------------- 公告 %s 已存在', ori_id)
            self.log.info('')
            return
        ...
        resp = WebRequest(self.log).post(url, cookies=cookies)
        self.log.info('---------------- HTTP_CODE: %s', resp.response.status_code)
    ...
    def parse_list_html(self, html):
        ...
        if len(div_content_lists) <= 0:
            self.log.info('================================ 列表第 %s 页未解析出公告数据', self.list_current_page)
            self.log.info('')
        ...
        self.log.info('================================ 列表第 %s 页共有公告 %s 条', self.list_current_page, len(content_list))

        for i, content in enumerate(content_list):
            self.log.info('---------------- 列表第 %s 页公告第 %s 条', self.list_current_page, i + 1)
    ...
    def request_list(self, cookies=None):
        ...
        resp = WebRequest(self.log).post(ebnewConst.LIST_URL, data=data, cookies=cookies)
        self.log.info('================================ HTTP_CODE: %s', resp.response.status_code)
        if resp.response.status_code == 521:
            # 如果响应的为反爬的js，则处理后再次请求
            cookie_jar = ebnewSpiderExtra.cookie_jar_from_521(resp.text)
        ...
        if expired_notice:
            ...
            self.log.info('================================ 已爬到列表最大页数 %s', ebnewConst.LIST_MAX_PAGE)
        else:
            # 整个列表都没有过期的公告，继续爬下一页
            self.list_current_page += 1
            self.log.info('================================ 请求列表第 %s 页', self.list_current_page)
            self.request_list()
        """
